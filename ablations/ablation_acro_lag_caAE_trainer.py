...
class Model(pl.LightningModule):
    ...
    def encode(self, batch_image):
        phi1 = self.recog_net_1(batch_image[:, 0:1].reshape(self.bs, self.d*self.d))
        phi1_n = phi1 / phi1.norm(dim=-1, keepdim=True)

        theta = self.get_theta(1, 0, self.link1_l*phi1_n[:, 1], self.link1_l*phi1_n[:, 0])
        grid = F.affine_grid(theta, torch.Size((self.bs, 1, self.d, self.d)))
        att_win = F.grid_sample(batch_image[:, 1:2], grid)
        phi2 = self.recog_net_2(att_win.reshape(self.bs, self.d*self.d))
        phi2_n = phi2 / phi2.norm(dim=-1, keepdim=True)
        return phi1, phi1_n, phi2, phi2_n
    ...
    def forward(self, X, u):
        ...
        # encode
        self.phi1_t0, self.phi1_n_t0, self.phi2_t0, self.phi2_n_t0 = self.encode(X[0])
        self.phi1_t1, self.phi1_n_t1, self.phi2_t1, self.phi2_n_t1 = self.encode(X[1])
        
        # estimate velocity
        self.phi1_dot_t0 = self.angle_vel_est(self.phi1_n_t0, self.phi1_n_t1, self.t_eval[1]-self.t_eval[0])
        self.phi2_dot_t0 = self.angle_vel_est(self.phi2_n_t0, self.phi2_n_t1, self.t_eval[1]-self.t_eval[0])

        # predict
        z0_u = torch.cat([self.phi1_n_t0[:, 0:1], self.phi2_n_t0[:, 0:1], self.phi1_n_t0[:, 1:2], self.phi2_n_t0[:, 1:2], 
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        norm_penalty = (self.phi1_t0.norm(dim=-1).mean() - 1) ** 2 + \
                       (self.phi2_t0.norm(dim=-1).mean() - 1) ** 2
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='ablation-acro-lag-caAE', type=str)
